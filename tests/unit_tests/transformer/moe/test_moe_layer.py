...
class TestMoELayerInit:
    ...
    @pytest.mark.skipif(
        not is_te_min_version("1.7.0.dev0") and is_real_cuda_device_available(),
    ...
    def test_legacy_moe_layer(self, num_moe_experts, moe_token_dispatcher_type, grouped_gemm):
        if grouped_gemm and not is_real_cuda_device_available():
            pytest.skip("legacy GroupedMLP not supported on HPU")
        Utils.initialize_model_parallel(1, 1)
        _set_random_seed(seed_=123, data_parallel_random_init=False)
        num_moe_experts = 4
        self.transformer_config = TransformerConfig(
