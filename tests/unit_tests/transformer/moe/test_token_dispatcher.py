...
class MoEModelTestContainer:
    def __init__(
        ...
        **kwargs,
        ...
        self.config = TransformerConfig(
            ...
            moe_capacity_bins_num=moe_capacity_bins_num,
    ...
    def dispatcher_drop_and_pad_test(self):
        ...
        if hasattr(moe_layer.token_dispatcher, "drop_and_pad"):
            moe_layer.token_dispatcher.drop_and_pad = False
        if hasattr(moe_layer.token_dispatcher, "fixed_expert_capacity"):
            moe_layer.token_dispatcher.fixed_expert_capacity = False
        ...
        if hasattr(moe_layer.token_dispatcher, "fixed_expert_capacity"):
            moe_layer.token_dispatcher.fixed_expert_capacity = True
    ...
    def capacity_bins_test(self):
        "Test if the tokens are dropped and padded correctly"
        moe_layer = self.moe_layer
        sequence_length = 1024
        hidden_states = torch.randn((sequence_length, moe_layer.config.hidden_size)).cuda()
        hidden_states.requires_grad = True
        ...
        torch.autograd.backward(forward_answer, forward_answer)
        backward_answer = hidden_states.grad.clone().detach()
        hidden_states.grad = None
        torch.cuda.synchronize()
        # End
        ...
        if hasattr(moe_layer.token_dispatcher, "fixed_expert_capacity"):
            moe_layer.token_dispatcher.fixed_expert_capacity = True
        moe_layer_2.config.moe_capacity_bins_num = 0
        moe_layer_2.config.moe_expert_capacity_factor = 4.0
        moe_layer_2.router.capacity_bins = None

        probs_2, indices_2 = moe_layer_2.router(hidden_states)
        (permuted_input_2, tokens_per_expert_2) = moe_layer_2.token_dispatcher.token_permutation(
            hidden_states, probs_2, indices_2
        )
        restored_hidden_states, restored_bias = moe_layer_2.token_dispatcher.token_unpermutation(
            permuted_input_2
        ...
        torch.distributed.barrier()
        assert tokens_per_expert.sum() > sequence_length, "Tokens were dropped."
        assert (
            tokens_per_expert.sum() < tokens_per_expert_2.sum()
        ), "Excessive padding due to bin sizes."
        assert torch.allclose(
            restored_hidden_states, forward_answer
        ), "Restored hidden states does not match"
