...
class TestBertModel:
    ...
    def setup_method(self, method):
        ...
        transformer_config = TransformerConfig(
            ...
            attention_backend=(
                AttnBackend.unfused if is_real_cuda_device_available() else AttnBackend.auto
            ),
        )
...
class TestBertModelAttentionDimensions:
    ...
    def test_transformer_engine_version_1_10(self, mocker):
        ...
        assert (
            attn_mask_dimensions == "b11s" if is_real_cuda_device_available() else "b1ss"
    ...
    def test_transformer_engine_version_1_7_to_1_10_unfused_attention(self, mocker):
        self.bert_model.config.attention_backend = (
            AttnBackend.unfused if is_real_cuda_device_available() else AttnBackend.auto
        )
        bert_layer_with_transformer_engine_spec.submodules.self_attention.params[
            'attn_mask_type'
        ...
        expected_attn_mask_type = AttnMaskType.arbitrary
        cuda_available = is_real_cuda_device_available()
        if not cuda_available:
            expected_attn_mask_type = AttnMaskType.padding
        assert (
            attn_mask_type == expected_attn_mask_type
        ), f"Exepcted attn mask type to be {expected_attn_mask_type}, but got {attn_mask_type}"
        assert (
            attn_mask_dimensions == "b1ss"
        ), f"Expected b1ss for attn_mask_dimensions but got {attn_mask_dimensions}"
