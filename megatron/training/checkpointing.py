...
def save_checkpoint(iteration, model, optimizer, opt_param_scheduler, num_floating_point_operations_so_far,
    ...
    if not torch.distributed.is_initialized() \
        ...
        if args.verify_checkpoint:
            ckpt_folder = os.path.join(save_dir, f"iter_{iteration:07d}")
...
def load_args_from_checkpoint(
    ...
    _set_arg('accumulate_allreduce_grads_in_fp32', force=True)
    _set_arg('fp8_coverage', force=True)
        ...
        if is_dist_ckpt:
            ...
            with contextlib.ExitStack() as stack:  # Allows multiple context managers for each model shard
                ...
                load_kwargs['sharded_state_dict'] = generate_state_dict(
                    ...
                    use_dist_ckpt=True,  iteration=iteration, optim_sd_kwargs=optim_sd_kwargs,
