...
_ALL_DATA_PARALLEL_RANKS_WITH_CP = None
...
# MOE token distribution logging
_MOE_TOKEN_DISTRIBUTION_LOGGING_TRACKER = {}
...
def initialize_model_parallel(
    ...
    global _ALL_DATA_PARALLEL_RANKS_WITH_CP
    assert _DATA_PARALLEL_GROUP is None, 'data parallel group is already initialized'
    assert _ALL_DATA_PARALLEL_RANKS is None, 'data parallel ranks are already initialized'
    assert (
        _ALL_DATA_PARALLEL_RANKS_WITH_CP is None
    ), 'data parallel ranks combined with context parallel are already initialized'
    ...
    all_data_parallel_ranks_with_cp = []
    for ranks_with_cp in generator_wrapper('dp-cp'):
        ...
        all_data_parallel_ranks_with_cp.append(ranks_with_cp)
    ...
    if len(all_data_parallel_ranks_with_cp) > 0:
        _ALL_DATA_PARALLEL_RANKS_WITH_CP = all_data_parallel_ranks_with_cp
...
def get_all_data_parallel_ranks(with_context_parallel=False):
    """Get all data-parallel groups."""
    if with_context_parallel:
        assert (
            _ALL_DATA_PARALLEL_RANKS_WITH_CP is not None
        ), 'data parallel groups with context parallel combined are not initialized'
        return _ALL_DATA_PARALLEL_RANKS_WITH_CP
    else:
        assert _ALL_DATA_PARALLEL_RANKS is not None, 'there are no data parallel groups initialized'
...
def get_moe_token_distribution_logging_tracker():
    """Return the moe token distribution tracker."""
    global _MOE_TOKEN_DISTRIBUTION_LOGGING_TRACKER
    return _MOE_TOKEN_DISTRIBUTION_LOGGING_TRACKER
