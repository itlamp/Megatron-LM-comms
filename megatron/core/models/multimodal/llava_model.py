...
from megatron.core.utils import is_real_cuda_device_available, log_single_rank
...
class LLaVAModel(MegatronModule):
    ...
    def __init__(
        ...
        if HAVE_TE and (self.sequence_parallel_lm or self.context_parallel_lm > 1):
            assert (
    ...
    def _preprocess_data(
        ...
        if self.post_process and has_labels:
            ...
            if not is_real_cuda_device_available():
                batch_range = batch_range.to(labels.device)
