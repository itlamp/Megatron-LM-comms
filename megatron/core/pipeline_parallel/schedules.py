...
def backward_step(input_tensor, output_tensor, output_tensor_grad, model_type, config):
    ...
    cag_context = contextlib.nullcontext()
    if config.enable_compiled_autograd:
        from torch._dynamo import compiled_autograd

        cag_context = compiled_autograd._enable(torch.compile(backend="hpu_backend", dynamic=False))
    ...
    *,
    ...
    with no_sync_func():
        for i in range(num_microbatches - 1):
            ...
            total_num_tokens += num_tokens.clone().detach()
            call_mark_step()
            if not forward_only:
                backward_step(input_tensor, output_tensor, output_tensor_grad, model_type, config)
            if (
                hasattr(config, "micro_batch_sync_interval")
                and config.micro_batch_sync_interval
                and i % config.micro_batch_sync_interval == 0
            ):
