...
class TransformerConfig(ModelParallelConfig):
    ...
    """When set to True, use TE fp8_smooth_swiglu"""
    ...
    """moe_capacity_bins_num (int): Number of capacity bins to use in case of
    moe_expert_capacity_factor = None. The default is 0."""

    moe_capacity_bins_exp_base: float = 1.5
    """moe_capacity_bins_exp_base (float): In case of capacity bins, exponential growing factor
    for bin width. The default is 1.5."""

    moe_capacity_bins_optimize_interval: int = 300
    """moe_capacity_bins_optimize_interval (int): Interval for capacity bins optimization.
    The default is 300."""

    moe_capacity_bins_optimize_max_group: int = 4
    """moe_capacity_bins_optimize_max_group (int): Maximum number of experts to be grouped for
    capacity bins optimization. The default is 4."""

    moe_capacity_bins_max_overhead_factor: float = 0.0
    """moe_capacity_bins_max_overhead_factor (float): Value of capacity bins overhead, that will
    trigger bins optimization. Overhead is defined as relative additional capacity used with bins,
    compared to requested capacity value. The default is 0.0."""

    moe_capacity_bins_alignment: int = 64
    """moe_capacity_bins_alignment (int): In case of capacity bins, required bins alignment.
    The default is 64."""

    moe_configured_bins: Optional[list] = None
    """moe_configured_bins (list, optional): Explicit configuration of capacity bin edges.
    The default is None."""
    ...
    """Training CPU-GPU synchronization at micro batch interval, to ensure that CPU is not running
    ...
    def __post_init__(self):
        ...
        if self.apply_rope_fusion:
            ...
            if (
                cuda_available
                and fused_apply_rotary_pos_emb is None
