...
from megatron.core.utils import is_real_cuda_device_available, make_viewless_tensor
...
class TransformerLayer(MegatronModule, BaseTransformerLayer):
    ...
    def __init__(
        ...
        if config.cp_comm_type is not None and is_real_cuda_device_available():
    ...
    def forward(
        ...
        input_layernorm_output = (
            self.input_layernorm(hidden_states) if self.use_pre_norm else hidden_states
        )
        ...
        pre_mlp_layernorm_output = (
            self.pre_mlp_layernorm(hidden_states) if self.use_pre_norm else hidden_states
        )
