...
class TEGroupedMLP(MegatronModule):
    ...
    def __init__(self, num_local_experts, config: TransformerConfig, submodules: MLPSubmodules):
        ...
        if self.config.fp8 and HAVE_TE:
...
class IntelDynamicMLP(MegatronModule):
    ...
    def __init__(self, num_local_experts: int, config: TransformerConfig):
        ...
        self.config = config
        self.num_local_experts = num_local_experts
        self.tp_size = parallel_state.get_expert_tensor_parallel_world_size()
        self.tp_rank = parallel_state.get_expert_tensor_parallel_rank()
        self.ep_rank = parallel_state.get_expert_model_parallel_rank()
        local_expert_indices_offset = self.ep_rank * self.num_local_experts
        self.local_expert_indices = list(
            range(local_expert_indices_offset, self.num_local_experts + local_expert_indices_offset)
        )
    ...
    def initialize_moe_weight(
        ...
        if self.config.perform_initialization:
            if self.config.use_cpu_initialization:
                ...
                _initialize_affine_weight_gpu(
                    weight, init_method=init_method, partition_dim=dim, is_expert=True
                )
