    ...
    # `BaseTransformerLayer` from the `transformer_layer.py` file.
        ...
        if issubclass(spec.module, TransformerBlock):
            ...
            if config.normalization not in ('LayerNorm', 'RMSNorm'):
                raise Exception(
                    f'Only LayerNorm and RMSNorm are currently supported, '
                    f'configured {config.normalization}'
                )
        ...
        def checkpoint_handler(forward_func):
            ...
            if self.config.fp8:
                if not is_real_cuda_device_available():
                    from intel_transformer_engine.distributed import checkpoint as te_checkpoint
                return te_checkpoint(  # pylint: disable=possibly-used-before-assignment
                    forward_func,
    ...
    def forward(
        ...
        if self.config.fp8:
            if is_real_cuda_device_available():
                ...
                f"Context Parallelism not supported with fp8 precision."
