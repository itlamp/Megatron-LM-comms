...
class DistributedOptimizer(MixedPrecisionOptimizer):
    ...
    def __init__(
        ...
        try:
            ...
            FusedAdamW = None
        assert (
            isinstance(optimizer, (Adam, FusedAdamW)) or optimizer is None
        ...
        if not HAVE_APEX_OR_TE:
            try:
                steps = list(
                    set(
                        [
                            s["step"].item() if isinstance(s["step"], torch.Tensor) else s["step"]
    ...
    def load_state_dict(self, state_dict, iteration=None):
        ...
        ** Note: Torch optimizer's state structure. **
        ...
        #   list.
        if iteration is not None and self.iteration is None:
        ...
        if not HAVE_APEX_OR_TE:
            ...
            for s in state_dict_state.values():
                ...
                s["step"] = step
            for state_dict_param_group in state_dict_param_groups:
                state_dict_param_group["step"] = step
    ...
    def _set_main_param_and_optimizer_states(self, model_param, tensors):
        ...
        if self.config.use_precision_aware_optimizer:
            ...
            for key in dst_tensors:
                if not key in tensors.keys():
                    continue
                if isinstance(dst_tensors[key], torch.Tensor):
                    dst_tensors[key].copy_(tensors[key])
                else:
                    dst_tensors[key] = tensors[key]
    ...
    def save_parameter_state_by_dp_groups(
        ...
        if save_distrib_optimizer_method == 'serial_per_dp_groups':
            serial_data_parallel_ranks = get_all_data_parallel_ranks(with_context_parallel=True)
        else:
        ...
        for dp_ranks in serial_data_parallel_ranks:
            if world_rank in dp_ranks:
                ...
                print_rank_0(
                    f'Started exporting distributed optimizer states for a data parallel group '
                    f'with `{data_parallel_global_ranks}` global ranks.',
                ...
                if data_parallel_rank == gather_data_parallel_rank:
                    ...
                    state_dict.clear()
                    print(
                        f'Distributed optimizer states has been saved for a data parallel group '
                        f'with `{data_parallel_global_ranks}` global ranks.'
                    )
                del state_dict
                gc.collect()
    ...
    def sharded_state_dict(
        ...
        if is_loading:
            ...
            if 'iteration' not in model_sharded_state_dict.keys():
                self.load_state_dict(self.state_dict())
            else:
                self.load_state_dict(self.state_dict(), model_sharded_state_dict['iteration'])
    ...
    def sharded_param_state_dp_zero(
        ...
        if not is_loading and self.distributed_optimizer_instance_id == 0:
            # Gather on rank 0
                    ...
                    for model_param, param_range_map in gbuf_range_map["param_map"].items():
                        ...
                        for state_key, state_ten in tensors.items():
                            replace_kwargs = dict(
                                ...
                                dtype=(
                                    state_ten.dtype
                                    if isinstance(state_ten, torch.Tensor)
                                    else type(state_ten)
                                ),
                            ...
                            if isinstance(state_ten, torch.Tensor) and state_key != "step":
        ...
        *,
        ...
        for dp_ranks in serial_data_parallel_ranks:
            if world_rank in dp_ranks:
                ...
                print_rank_0(
                    f'Started loading distributed optimizer states for a data parallel group '
                    f'with `{data_parallel_global_ranks}` global ranks.',
                    self.data_parallel_group,
                )
                ...
                if data_parallel_rank == scatter_data_parallel_rank:
                    ...
                    print(
                        f'Distributed optimizer states has been loaded for a data '
                        f'parallel group with `{data_parallel_global_ranks}` global ranks.'
                    )
                del state_dict
                gc.collect()
