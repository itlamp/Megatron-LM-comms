...
def get_te_version():
    ...
    global _te_version
    if _te_version is None and is_real_cuda_device_available():
...
class GlobalMemoryBuffer:
    ...
    def get_tensor_without_assignment(self, tensor_shape, dtype, name):
        ...
        if (
            ...
            return buffer.view(*tensor_shape)
        return self.buffer[(name, dtype)][0:required_len].view(*tensor_shape)
...
def get_node_data_parallel_ranks(data_parallel_global_ranks):
    ...
    all_dp_global_ranks = parallel_state.get_all_data_parallel_ranks(with_context_parallel=True)

    # Group ranks by nodes
    if len(all_dp_global_ranks) > num_nodes:
        num_groups_per_node = len(all_dp_global_ranks) // num_nodes
    else:
        num_groups_per_node = 1

    # Determine the rank ranges for data parallel ranks associated with nodes.
    all_nodes_ranks = []
    for node_id in range(num_nodes):
        ranks = list(range(device_count * node_id, device_count * node_id + device_count))
        all_nodes_ranks.append(ranks)

    node_dp_groups_counter = [0] * num_nodes
    all_node_dp_global_ranks = [None] * num_nodes

    # Assign data parallel groups to nodes
    for dp_global_ranks in all_dp_global_ranks:
        for node_id, node_ranks in enumerate(all_nodes_ranks):
            if node_dp_groups_counter[node_id] >= num_groups_per_node:
                continue
            if len(list(set(dp_global_ranks).intersection(node_ranks))) > 0:
                if all_node_dp_global_ranks[node_id] == None:
                    all_node_dp_global_ranks[node_id] = [dp_global_ranks]
                else:
                    all_node_dp_global_ranks[node_id].append(dp_global_ranks)
                node_dp_groups_counter[node_id] += 1
                break

    # Find node_id for a given data parallel group
    node_id = None
    for i in range(num_nodes):
        if all_node_dp_global_ranks[i] is None:
            continue
        if data_parallel_global_ranks in all_node_dp_global_ranks[i]:
            node_id = i
            break

    assert node_id is not None

    node_dp_global_ranks = all_node_dp_global_ranks[node_id]
    node_ranks = all_nodes_ranks[node_id]

    return node_dp_global_ranks, node_ranks
