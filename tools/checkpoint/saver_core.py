...
def save_checkpoint(queue, args):
    ...
    if hasattr (md, 'checkpoint_args'):
        ...
        args_to_keep = ['tensor_model_parallel_size', 'pipeline_model_parallel_size', 'expert_model_parallel_size', 'world_size', 'params_dtype',
                        ...
                        'ckpt_format', 'main_grads_dtype', 'main_params_dtype', 'exp_avg_dtype',
                        'exp_avg_sq_dtype',
        ]
        ...
        for layer_id in range(schema.get_num_layers(models[pp_rank][0][0])):
            ...
            for ep_rank in range(args.target_expert_parallel_size):
                for tp_rank in range(args.target_tensor_parallel_size):
                    ...
                    if margs.num_experts:
                        ...
                        for expert_idx in range(num_local_experts):
                            if margs.moe_dynamic_hpu:
                                offsetted_expert_idx = ep_rank * num_local_experts + expert_idx
                            else:
                                offsetted_expert_idx = expert_idx
                            params_dict.update({
                                f"mlp_fc1_weight.{offsetted_expert_idx}" : mlp_l0_weight[ep_rank][tp_rank][expert_idx],
                                f"mlp_fc2_weight.{offsetted_expert_idx}" : mlp_l1_weight[ep_rank][tp_rank][expert_idx],
                            })
                    else:
                        params_dict.update({
                    ...
                    if margs.num_experts:
                        if margs.moe_dynamic_hpu and ep_rank > 0:
                            num_local_experts = margs.num_experts // args.target_expert_parallel_size

                            for expert_idx in range(num_local_experts):
                                offsetted_expert_idx = ep_rank * num_local_experts + expert_idx

                                expert_fc1 = f'local_expert_{expert_idx}_linear_fc1_weight'
                                expert_fc2 = f'local_expert_{expert_idx}_linear_fc2_weight'

                                experts = model.decoder.layers[layer_id].mlp.experts

                                linear_fc1 = getattr(experts, expert_fc1)
                                linear_fc2 = getattr(experts, expert_fc2)

                                offsetted_expert_fc1 = f'local_expert_{offsetted_expert_idx}_linear_fc1_weight'
                                offsetted_expert_fc2 = f'local_expert_{offsetted_expert_idx}_linear_fc2_weight'

                                setattr(experts, offsetted_expert_fc1, copy.deepcopy(linear_fc1))
                                setattr(experts, offsetted_expert_fc2, copy.deepcopy(linear_fc2))

                                delattr(experts, expert_fc1)
                                delattr(experts, expert_fc2)
